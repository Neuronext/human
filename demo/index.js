...
document.getElementById('recordMuse').addEventListener('click', function() {
  ...
  .then(data => {
    ...
    }, 10000); // Delay countdown start by 10 seconds
  })
  .catch(error => {
...
function checkRecordingStatus(filename, button) {
  fetch(`http://localhost:3000/status/${filename}`)
    ...
    .then(status => {
      ...
      if (status.status === 'Completed') {
        ...
        // Retry checking the status after a delay
        setTimeout(() => checkRecordingStatus(filename, button), 1000);
      }
    })
    .catch(error => {
