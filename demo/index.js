...
let currentChunk = [];
const maxChunkSize = 10000000; // Threshold for flushing data into Blob
...
function addToResults(result) {
  ...
  if (currentChunk.length >= maxChunkSize) {
      accumulatedBlobParts = currentChunk;
      currentChunk = []; 
  }
}
...
function downloadCSV(data, filename = 'data.csv') {
  const csvString = [Object.keys(data[0]).join(','), ...data.map(row => Object.values(row).join(','))].join('\n');
