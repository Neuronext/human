...
async function addFaces(imgEl) {
  ...
  // const csvString = resultsToCsv(res);
  // downloadCsv(csvString, 'detection-results.csv');
  ...
  //   const imgEl = document.getElementById('yourImageElementId');
  //   addFaces(imgEl).then(() => {
  //     const csvString = resultsToCsv(lastDetectedResults); 
...
//   const csvRows = ['source,score,age,gender,emotion,rotation,gaze,distance,check']; // CSV Header
//   results.face.forEach((face) => {
//     const row = [
//       `"${face.source}"`,
//       face.boxScore.toFixed(2),
//       face.age,
//       `"${face.gender}"`,
//       `"${face.emotion?.map(e => `${e.emotion} (${e.score.toFixed(2)})`).join('; ')}"`,
//       `"pitch: ${face.rotation?.angle.pitch.toFixed(2)}, roll: ${face.rotation?.angle.roll.toFixed(2)}, yaw: ${face.rotation?.angle.yaw.toFixed(2)}"`,
//       `"direction: ${face.rotation?.gaze.bearing.toFixed(2)}, strength: ${(face.rotation.gaze.strength || 0).toFixed(2)}"`,
//       face.distance.toFixed(2),
//       `real: ${face.real.toFixed(2)}, live: ${face.live.toFixed(2)}`,
//     ].join(',');
//     csvRows.push(row);
//   });
//   return csvRows.join('\n');
...
// function downloadCsv(csvString, filename = 'detection-results.csv') {
